...
export class AgendaController {
  ...
  async getAll(): Promise<ReturnAgendaDTO[]> {
    return (await this.agendaService.getAll()).map(
      (agenda) => new ReturnAgendaDTO(agenda),
    );
  }
  ...
  async getById(@Param('id') id: string): Promise<ReturnAgendaDTO> {
    return new ReturnAgendaDTO(await this.agendaService.getById(id));
  }
  ...
  async update(
    ...
    return new ReturnAgendaDTO(await this.agendaService.update(id, data));
  }
